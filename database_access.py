...
def add_comment(comment,uid,professer,course_code,rating):
    try:
        ...
        return({"status":"success","comment_id":str(comments_id.inserted_id)})
...
def del_comment(comment_id,operator):
    try:
        ...
        comment_data = list(collection.find({"_id":comment_id},{"comment":1,"rating":1,"course_code":1}))

        if len(comment_data) == 0:
            return("Comment_not_found")
        comment = comment_data[0]["comment"]
        rating = comment_data[0]["rating"]
        course_code = comment_data[0]["course_code"]
        collection.delete_one({"_id":comment_id})
        change_log = {"comment_id":comment_id,"comment":comment,"rating":rating,"course_code":course_code}
...
def search_comment(type,target):
# search_comments can support three types of target:
#   course_code
#   uid
#   operator
#   comment_id
...
def get_comment_by_id(comment_id):
    ...
    db = client.comments
    collection = db.comments
    comment = list(collection.find({"_id":ObjectId(comment_id)}))[0]
    comment_id = comment.pop("_id")
    comment["comment_id"] = str(comment_id)
    print(comment)
    return comment
...
# this function will return a uid if target is a uuid
    ...
    client = pymongo.MongoClient(os.getenv("MONGODB_URL"),uuidRepresentation='standard')
    db = client.comments
